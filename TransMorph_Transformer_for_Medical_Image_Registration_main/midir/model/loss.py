...
class LNCCLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        cov = torch.clamp(xy_sum - y_mu * x_sum - x_mu * y_sum + x_mu * y_mu * window_num_points,
                          min=self.num_stab_const)
        x_var = torch.clamp(xsq_sum - 2 * x_mu * x_sum + x_mu * x_mu * window_num_points, min=self.num_stab_const)
        y_var = torch.clamp(ysq_sum - 2 * y_mu * y_sum + y_mu * y_mu * window_num_points, min=self.num_stab_const)
